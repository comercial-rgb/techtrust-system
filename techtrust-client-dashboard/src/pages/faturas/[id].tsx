...
export default function InvoiceDetailPage() {
  ...
  const getStatusInfo = (status: string) => {
    const statuses: Record<string, { label: string; color: string; bg: string }> = {
      ...
      COMPLETED: { label: 'Awaiting Your Review', color: 'text-yellow-700', bg: 'bg-yellow-100' },
      APPROVED: { label: 'Approved', color: 'text-green-700', bg: 'bg-green-100' },
      DISPUTED: { label: 'Disputed', color: 'text-red-700', bg: 'bg-red-100' },
    };
    return statuses[status] || { label: status, color: 'text-gray-700', bg: 'bg-gray-100' };
  };
  ...
  return (
    <DashboardLayout title={t('client.invoices.detail') || 'Invoice Detail'}>
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            ...
            <div className="text-right">
              <p className="text-3xl font-bold text-gray-900">{formatCurrency(invoice.finalTotal)}</p>
              {Number(invoice.supplementsTotal) > 0 && (
                <p className="text-sm text-blue-600 mt-1">
                  Includes {formatCurrency(invoice.supplementsTotal)} in supplements
                </p>
              )}
            </div>
          </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-yellow-700 mb-4">
                  {t('client.invoices.reviewDesc') || 'This repair has been completed. Please review the invoice and either approve or dispute it.'}
                </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-2 text-sm">
              <p><span className="text-gray-500">Name:</span> <span className="font-medium">{invoice.providerName}</span></p>
              {invoice.providerBusinessName && (
                <p><span className="text-gray-500">Business:</span> <span className="font-medium">{invoice.providerBusinessName}</span></p>
              )}
              {invoice.providerMvrLicense && (
                <p><span className="text-gray-500">MVR License:</span> <span className="font-medium">{invoice.providerMvrLicense}</span></p>
              )}
              {invoice.providerPhone && (
                <p><span className="text-gray-500">Phone:</span> <span className="font-medium">{invoice.providerPhone}</span></p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-2 text-sm">
              <p><span className="text-gray-500">Vehicle:</span> <span className="font-medium">{invoice.vehicleInfo}</span></p>
              {invoice.odometerIn && (
                <p><span className="text-gray-500">Odometer In:</span> <span className="font-medium">{invoice.odometerIn} mi</span></p>
              )}
              {invoice.odometerOut && (
                <p><span className="text-gray-500">Odometer Out:</span> <span className="font-medium">{invoice.odometerOut} mi</span></p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            {invoice.laborChargeType && (
              <div>
                ...
                <p className="font-medium capitalize">{invoice.laborChargeType.replace('_', ' ').toLowerCase()}</p>
                {invoice.hourlyRate && <p className="text-xs text-gray-400">${invoice.hourlyRate}/hr</p>}
              </div>
            )}
            {Number(invoice.shopSuppliesFee) > 0 && (
              <div>
                <p className="text-gray-500">Shop Supplies Fee</p>
                <p className="font-medium">{formatCurrency(invoice.shopSuppliesFee)}</p>
              </div>
            )}
            ...
            {Number(invoice.batteryFee) > 0 && (
              <div>
                ...
                <p className="font-medium">{formatCurrency(invoice.batteryFee)}</p>
              </div>
            )}
            {Number(invoice.dailyStorageCharge) > 0 && (
              <div>
                <p className="text-gray-500">Daily Storage Charge</p>
                <p className="font-medium">{formatCurrency(invoice.dailyStorageCharge)}/day</p>
              ...
              <table className="w-full min-w-[600px]">
                <thead>
                  <tr className="bg-gray-50 text-sm text-gray-500">
                    ...
                    <th className="text-left px-6 py-3 font-medium">Condition</th>
                    ...
                    <tr key={item.id || index} className="text-sm">
                      <td className="px-6 py-3">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                          item.type === 'PART' ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700'
                        }`}>
                          {item.type}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-3 text-gray-600">
                        {item.partCondition === 'NEW' ? 'New' :
                         item.partCondition === 'USED' ? 'Used' :
                         item.partCondition === 'REBUILT' ? 'Rebuilt' :
                         item.partCondition === 'RECONDITIONED' ? 'Reconditioned' : '-'}
                      </td>
                      ...
                      <td className="px-6 py-3 text-right font-medium text-gray-900">{formatCurrency(item.totalPrice)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* Totals */}
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h3 className="font-semibold text-gray-900 mb-4">{t('client.invoices.totals') || 'Totals'}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {invoice.servicePerformed && (
              <div className="bg-white rounded-xl p-6 shadow-sm">
                ...
                <p className="text-sm text-gray-700 whitespace-pre-wrap">{invoice.servicePerformed}</p>
              </div>
            )}
            {invoice.warrantyDescription && (
              <div className="bg-white rounded-xl p-6 shadow-sm">
                ...
                <p className="text-sm text-gray-700 whitespace-pre-wrap">{invoice.warrantyDescription}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 text-xs text-blue-800">
          <h4 className="font-semibold mb-2">Florida Motor Vehicle Repair Act Notice</h4>
          <ul className="space-y-1 list-disc list-inside">
            <li>Florida Statute §559.905 requires a written estimate for repairs over $150.</li>
            <li>You may not be charged more than 10% above the estimate without written authorization.</li>
            <li>Replaced parts must be returned upon request (except warranty or exchange parts).</li>
            <li>You have the right to inspect replaced parts before they are disposed of.</li>
          </ul>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <p className="text-sm text-gray-500 mb-4">
              {t('client.invoices.disputeDesc') || 'Please describe the reason for your dispute. The provider will be notified.'}
            </p>
            <textarea
              value={disputeReason}
              onChange={(e) => setDisputeReason(e.target.value)}
              placeholder={t('client.invoices.disputePlaceholder') || 'Describe the issue...'}
              rows={4}
            ...
            <div className="flex gap-3">
              ...
              >
                {actionLoading ? 'Sending...' : t('client.invoices.submitDispute') || 'Submit Dispute'}
              </button>
            </div>
          </div>
        </div>
