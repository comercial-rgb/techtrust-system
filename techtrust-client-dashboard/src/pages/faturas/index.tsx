...
export default function InvoicesPage() {
  ...
  const getStatusInfo = (status: string) => {
    ...
    > = {
      DRAFT: {
        label: t("client.invoices.statusDraft") || "Draft",
        color: "bg-gray-100 text-gray-700",
        icon: <FileText className="w-4 h-4" />,
      },
      IN_PROGRESS: {
        label: t("client.invoices.statusInProgress") || "In Progress",
        color: "bg-blue-100 text-blue-700",
        icon: <Clock className="w-4 h-4" />,
      },
      COMPLETED: {
        label: t("client.invoices.statusCompleted") || "Awaiting Review",
        color: "bg-yellow-100 text-yellow-700",
        icon: <CheckCircle className="w-4 h-4" />,
      },
      APPROVED: {
        label: t("client.invoices.statusApproved") || "Approved",
        color: "bg-green-100 text-green-700",
        icon: <CheckCircle className="w-4 h-4" />,
      },
      DISPUTED: {
        label: t("client.invoices.statusDisputed") || "Disputed",
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <p className="text-sm text-gray-500">
              {t("client.invoices.statTotal") || "Total"}
            </p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <p className="text-sm text-gray-500">
              {t("client.invoices.statNeedsReview") || "Needs Review"}
            </p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <p className="text-sm text-gray-500">
              {t("client.invoices.statApproved") || "Approved"}
            </p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <p className="text-sm text-gray-500">
              {t("client.invoices.statTotalPaid") || "Total Paid"}
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl p-12 shadow-sm text-center">
            ...
            <p className="text-gray-500">
              {t("client.invoices.emptyDesc") ||
                "Invoices will appear here once a service is completed."}
            </p>
          </div>
        ) : (
          <div className="space-y-4">
                      ...
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span
                              className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full ${statusInfo.color}`}
                            >
                              {statusInfo.label}
                            </span>
                          </div>
