...
export default function EstimatesPage() {
  ...
  const getStatusInfo = (status: string) => {
    ...
    > = {
      PENDING: {
        label: t("common.status.pending") || "Pending",
        color: "bg-yellow-100 text-yellow-700",
        icon: <Clock className="w-4 h-4" />,
      },
      ACCEPTED: {
        label: t("common.status.accepted") || "Accepted",
        color: "bg-green-100 text-green-700",
        icon: <CheckCircle className="w-4 h-4" />,
      },
      REJECTED: {
        label: t("common.status.rejected") || "Rejected",
        color: "bg-red-100 text-red-700",
        icon: <XCircle className="w-4 h-4" />,
      },
      EXPIRED: {
        label: t("common.status.expired") || "Expired",
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <p className="text-sm text-gray-500">
              {t("client.estimates.total") || "Total"}
            </p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder={
                t("client.estimates.search") || "Search estimates..."
              }
              value={searchQuery}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl p-12 shadow-sm text-center">
            ...
            <p className="text-gray-500 mb-6">
              {t("client.estimates.emptyDesc") ||
                "Create a service request to receive estimates from providers."}
            </p>
            <Link
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
                        <span className="text-xs text-gray-400">
                          #{quote.serviceRequestNumber}
                        </span>
                        <button
                          onClick={() =>
                            router.push(
                              `/solicitacoes/${quote.serviceRequestId}`,
                            )
                          }
