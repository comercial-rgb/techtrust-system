...
export default function FaturaDetalhePage() {
  ...
  return (
    <DashboardLayout
      title={`${t("invoices.detail")} - ${invoice.invoiceNumber}`}
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl p-5 shadow-soft">
            ...
            {invoice.odometerReading && (
              <p className="text-sm text-gray-500">
                {t("invoices.odometer")}:{" "}
                {invoice.odometerReading.toLocaleString()} mi
              </p>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl p-5 shadow-soft">
            ...
            <p className="text-sm text-gray-700">
              {t("invoices.registrationNumber")}:{" "}
              {invoice.fdacsRegistrationNumber || "N/A"}
            </p>
            <p className="text-sm text-gray-500">
              {t("invoices.quoteRef")}: {invoice.quote?.quoteNumber || "-"}
            </p>
            <p className="text-sm text-gray-500">
              {t("invoices.workOrderRef")}:{" "}
                  ...
                  return (
                    <tr key={idx}>
                      <td className="px-5 py-3 text-sm">
                        <span
                          className={`badge ${item.type === "LABOR" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700"}`}
                        >
                          {item.type || "PART"}
                        </span>
                      </td>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-lg font-semibold">
                ${Number(invoice.finalPartsCost).toFixed(2)}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-500">{t("invoices.labor")}</p>
              <p className="text-lg font-semibold">
                ${Number(invoice.finalLaborCost).toFixed(2)}
              </p>
            </div>
          ...
          !canEdit && (
            <div className="bg-white rounded-xl p-5 shadow-soft">
              ...
              <div className="flex gap-4 text-sm text-gray-500">
                {invoice.warrantyMonths && (
                  <span>{invoice.warrantyMonths} months</span>
                )}
                {invoice.warrantyMileage && (
                  <span>{invoice.warrantyMileage.toLocaleString()} miles</span>
                )}
              </div>
            </div>
          )}
