  ...
  try {
    ...
    if (!entityType || !entityId || !newStatus) {
      return res
        ...
        .json({
          success: false,
          message: "entityType, entityId, and newStatus are required",
        });
    }
    ...
    if (entityType === "COMPLIANCE_ITEM") {
      ...
      if (!item)
        return res
          .status(404)
          .json({ success: false, message: "Compliance item not found" });
      ...
      if (!policy)
        return res
          .status(404)
          .json({ success: false, message: "Insurance policy not found" });
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid entityType" });
    }
...
// ============================================
export const acceptRiskDisclaimer = async (
  req: Request,
  res: Response,
): Promise<any> => {
  try {
    ...
    if (!providerId || !serviceType) {
      return res
        ...
        .json({
          success: false,
          message: "providerId and serviceType are required",
        });
    }
...
async function recalculateProviderStatus(entityType: string, entityId: string) {
  try {
    ...
    const hasExpired =
      profile.complianceItems.some((c) => c.status === "EXPIRED") ||
    ...
    if (offersAC) {
      const verifiedEPATechs = profile.technicians.filter(
        (t) => t.epa609Status === "VERIFIED" && t.isActive,
      );
      if (verifiedEPATechs.length === 0) hasRestrictions = true;
    }
    if (offersTowing) {
      const comAuto = profile.insurancePolicies.find(
      ...
      if (
        comAuto?.status !== "INS_VERIFIED" ||
        onHook?.status !== "INS_VERIFIED"
      )
        hasRestrictions = true;
    }

    let newStatus: "VERIFIED" | "PENDING" | "RESTRICTED" | "NOT_ELIGIBLE" =
      "PENDING";
    if (hasExpired) {
