...
router.get(
  "/pending",
  asyncHandler(verificationController.getPendingVerifications),
);

// GET /verification/logs/:entityId - Get verification history
router.get(
  "/logs/:entityId",
  asyncHandler(verificationController.getVerificationLogs),
);

// POST /verification/risk-acceptance - Customer: Accept risk disclaimer
router.post(
  "/risk-acceptance",
  asyncHandler(verificationController.acceptRiskDisclaimer),
);

// GET /verification/risk-acceptance/check - Customer: Check risk acceptance status
router.get(
  "/risk-acceptance/check",
  asyncHandler(verificationController.checkRiskAcceptance),
);
