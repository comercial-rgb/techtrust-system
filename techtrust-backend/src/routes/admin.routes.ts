  ...
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    const [
      ...
      prisma.serviceRequest.count({
        where: {
          status: {
            in: ["SEARCHING_PROVIDERS", "QUOTES_RECEIVED", "IN_PROGRESS"],
          },
        },
      }),
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (verified !== undefined) {
      filteredProviders = providers.filter(
        (p) => p.providerProfile?.isVerified === (verified === "true"),
      );
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      message: `Fornecedor suspenso por ${days || "tempo indeterminado"} dias`,
      reason,
    });
  }),
);
...
router.post(
  "/providers/:id/verify-credentials",
  asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
...
router.post(
  "/service-requests/:id/assign-provider",
  asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    const countMap = new Map(
      subscriptionCounts.map((s) => [s.plan, s._count.id]),
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Get max position
    const maxPos = await prisma.subscriptionPlanTemplate.aggregate({
      _max: { position: true },
    });
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (type === "services") {
      ...
      res.json({
        ...
        byType,
        conversionRate:
          total > 0 ? ((conversionRate / total) * 100).toFixed(2) : 0,
      });
      return;
    }
