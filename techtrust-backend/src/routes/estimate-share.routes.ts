...
router.get(
  "/available",
  asyncHandler(estimateShareController.getAvailableSharedEstimates),
);

// Get shared estimate detail
router.get(
  "/:id",
  asyncHandler(estimateShareController.getSharedEstimateDetail),
);

// Submit competing quote (provider)
router.post(
  "/:id/submit-quote",
  asyncHandler(estimateShareController.submitCompetingQuote),
);
