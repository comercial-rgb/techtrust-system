...
export default function ProviderComplianceBadge({
  ...
  const content = (
    <View style={[styles.container, { backgroundColor: badge.bgColor }, size === 'large' && styles.containerLarge]}>
      <Ionicons name={badge.icon as any} size={iconSize} color={badge.color} />
      {showLabel && (
        <Text style={[styles.label, { color: badge.color, fontSize }]}>{badge.label}</Text>
      )}
    </View>
  );
