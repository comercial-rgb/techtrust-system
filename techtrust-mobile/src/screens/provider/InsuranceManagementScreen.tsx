...
export default function InsuranceManagementScreen({ navigation }: any) {
        ...
        ALL_INSURANCE_TYPES.forEach(type => {
          ...
          formState[type] = {
            ...
            expirationDate: existing?.expirationDate
              ...
              : '',
            coverageAmount: existing?.coverageAmount ? String(existing.coverageAmount) : '',
  ...
  const handleUpload = async (type: string) => {
    try {
      ...
      if (uploadRes.data?.url) {
        updateForm(type, 'coiUploads', [...(forms[type]?.coiUploads || []), uploadRes.data.url]);
      }
    } catch (error) {
  ...
  const handleSave = async (type: string) => {
    ...
    if (form.hasCoverage && (!form.carrierName || !form.policyNumber)) {
      Alert.alert('Missing Info', 'Carrier name and policy number are required when coverage is declared.');
      return;
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <Text style={styles.infoText}>
          Declare your insurance coverage for each type. Upload a Certificate of Insurance (COI) for verification.
        </Text>
        ...
        {ALL_INSURANCE_TYPES.map(type => {
          ...
          const statusInfo = existing ? getInsuranceStatusLabel(existing.status) : null;
          ...
          return (
            <View key={type} style={styles.card}>
              ...
              >
                <Ionicons
                  name={form?.hasCoverage ? 'shield-checkmark' : 'shield-outline'}
                  size={22}
                  color={form?.hasCoverage ? '#16a34a' : '#9ca3af'}
                />
                <View style={styles.cardTitleWrap}>
                  <Text style={styles.cardTitle}>{INSURANCE_TYPE_LABELS[type]}</Text>
                  {statusInfo && (
                  ...
                  {form.hasCoverage && (
                    <>
                      <View style={styles.field}>
                        <Text style={styles.fieldLabel}>Carrier / Insurer Name *</Text>
                        <TextInput
                          style={styles.input}
                          value={form.carrierName}
                          onChangeText={v => updateForm(type, 'carrierName', v)}
                      ...
                      <View style={styles.field}>
                        ...
                        <TextInput
                          ...
                          onChangeText={v => updateForm(type, 'policyNumber', v)}
                      ...
                      <View style={styles.field}>
                        ...
                        <TextInput
                          ...
                          onChangeText={v => updateForm(type, 'expirationDate', v)}
                      ...
                      <View style={styles.field}>
                        ...
                        <TextInput
                          ...
                          onChangeText={v => updateForm(type, 'coverageAmount', v)}
                      ...
                      <View style={styles.field}>
                        <Text style={styles.fieldLabel}>Certificate of Insurance (COI)</Text>
                        {form.coiUploads.map((url, i) => (
                          <View key={i} style={styles.docRow}>
                            <Ionicons name="document-attach" size={16} color="#1976d2" />
                            <Text style={styles.docText}>COI Document {i + 1}</Text>
                            <TouchableOpacity onPress={() => {
                              const newUploads = form.coiUploads.filter((_, idx) => idx !== i);
...
const styles = StyleSheet.create({
  ...
  infoText: { fontSize: 13, color: '#6b7280', marginBottom: 16, lineHeight: 20 },
  card: {
    backgroundColor: '#fff',
    borderRadius: 12,
    marginBottom: 10,
