...
export default function ComplianceItemDetailScreen({ route, navigation }: any) {
  ...
  const [documentUploads, setDocumentUploads] = useState<string[]>(item.documentUploads || []);
  ...
  const removeDocument = (index: number) => {
    Alert.alert('Remove Document', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Remove',
        style: 'destructive',
        onPress: () => setDocumentUploads(prev => prev.filter((_, i) => i !== index)),
      },
    ]);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{COMPLIANCE_TYPE_LABELS[item.type] || item.type}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={styles.field}>
          <Text style={styles.fieldLabel}>
            {item.type === 'FDACS_MOTOR_VEHICLE_REPAIR' ? 'FDACS Registration Number' : 'License/Registration Number'}
          </Text>
          <TextInput
            style={styles.input}
        ...
        <View style={styles.field}>
          ...
          <TextInput
            ...
            placeholderTextColor="#9ca3af"
          />
          <Text style={styles.fieldHint}>Format: YYYY-MM-DD (e.g., 2026-12-31)</Text>
        </View>

        {/* Documents */}
        <View style={styles.field}>
          <Text style={styles.fieldLabel}>Supporting Documents</Text>
          <Text style={styles.fieldHint}>Upload license, registration, or certificate images/PDFs</Text>
          ...
          <TouchableOpacity style={styles.uploadBtn} onPress={handleUploadDocument} disabled={uploading}>
            {uploading ? (
