...
export default function TechnicianManagementScreen({ navigation }: any) {
  ...
  const openEditModal = (tech: Technician) => {
    ...
    setEpa609CertExpiry(tech.epa609CertExpiry ? new Date(tech.epa609CertExpiry).toISOString().split('T')[0] : '');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = {
        ...
        epa609CertUploads: epa609CertUploads.length > 0 ? epa609CertUploads : undefined,
      };
      ...
      resetForm();
      await fetchData();
      Alert.alert('Success', editingTech ? 'Technician updated' : 'Technician added');
    } catch (error: any) {
      ...
      >
        {activeTechs.length === 0 ? (
          ...
          activeTechs.map(tech => {
            ...
            return (
              <TouchableOpacity key={tech.id} style={styles.card} onPress={() => openEditModal(tech)}>
                <View style={styles.cardHeader}>
                  ...
                  <View style={styles.cardInfo}>
                    ...
                    <Text style={styles.cardRole}>{ROLES.find(r => r.value === tech.role)?.label || tech.role}</Text>
                  </View>
                ...
                {tech.epa609CertNumber && (
                  <Text style={styles.certDetail}>Cert #: {tech.epa609CertNumber}</Text>
                )}
                {tech.epa609CertType && (
                  <Text style={styles.certDetail}>
                    Type: {EPA_CERT_TYPES.find(t => t.value === tech.epa609CertType)?.label || tech.epa609CertType}
                  </Text>
                )}
                {tech.epa609CertExpiry && (
                  <Text style={styles.certDetail}>
                    Expires: {new Date(tech.epa609CertExpiry).toLocaleDateString()}
                  </Text>
                )}
              </TouchableOpacity>
            );
        ...
        {inactiveTechs.length > 0 && (
          <View style={{ marginTop: 20 }}>
            <Text style={styles.sectionLabel}>Inactive ({inactiveTechs.length})</Text>
            {inactiveTechs.map(tech => (
              <View key={tech.id} style={[styles.card, { opacity: 0.5 }]}>
                <View style={styles.cardHeader}>
                  ...
                  <View style={styles.cardInfo}>
                    <Text style={[styles.cardName, { color: '#9ca3af' }]}>{tech.fullName}</Text>
      ...
      <Modal visible={modalVisible} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            ...
            <Text style={styles.modalTitle}>{editingTech ? 'Edit Technician' : 'Add Technician'}</Text>
            <TouchableOpacity onPress={handleSave} disabled={saving}>
              {saving ? <ActivityIndicator size="small" color="#1976d2" /> : <Text style={styles.saveBtn}>Save</Text>}
            </TouchableOpacity>
          </View>
            ...
            <View style={styles.field}>
              ...
              <View style={styles.roleGrid}>
                {EPA_CERT_TYPES.map(ct => (
                  <TouchableOpacity
                    key={ct.value}
                    style={[styles.roleChip, epa609CertType === ct.value && styles.roleChipActive]}
                    onPress={() => setEpa609CertType(ct.value)}
                  >
                    <Text style={[styles.roleChipText, epa609CertType === ct.value && styles.roleChipTextActive]}>
                      {ct.label}
                    </Text>
                  </TouchableOpacity>
                ))}
            ...
            <View style={styles.field}>
              ...
              {epa609CertUploads.map((url, i) => (
                <View key={i} style={styles.docRow}>
                  ...
                  <TouchableOpacity onPress={() => setEpa609CertUploads(prev => prev.filter((_, idx) => idx !== i))}>
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 14, fontWeight: '700', color: '#6b7280', marginBottom: 8, marginTop: 4 },
  emptyCard: {
    backgroundColor: '#fff',
    borderRadius: 12,
    padding: 32,
  ...
  emptyText: { fontSize: 14, color: '#6b7280', marginTop: 12, marginBottom: 16 },
  primaryBtn: { backgroundColor: '#1976d2', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 10 },
