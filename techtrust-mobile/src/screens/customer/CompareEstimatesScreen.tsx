      ...
      >
        ...
        <View style={styles.vehicleCard}>
          ...
          <Text style={styles.vehicleText}>
            {share.vehicleInfo || t.fdacs.vehicle}
          </Text>
          <View
            style={[
              styles.statusBadge,
              {
                backgroundColor:
                  share.status === "OPEN" ? "#dbeafe" : "#f3f4f6",
              },
            ]}
          >
            <Text
              style={[
                styles.statusText,
                { color: share.status === "OPEN" ? "#3b82f6" : "#6b7280" },
              ]}
            >
              {share.status}
            </Text>
        ...
        <View style={[styles.estimateCard, styles.originalCard]}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate("QuoteDetails", { quoteId: original?.id })
            }
          >
        ...
        {competing.length === 0 ? (
          ...
          competing.map((cq: any, idx: number) => {
            const savings =
              Number(original?.totalAmount || 0) - Number(cq.totalAmount || 0);
            ...
            return (
              <View key={cq.id || idx} style={styles.estimateCard}>
                <View style={styles.estimateHeader}>
                  <View>
                    <Text style={styles.estimateNumber}>
                      {cq.estimateNumber || `Quote #${idx + 1}`}
                    </Text>
                    <Text style={styles.providerName}>
                      {cq.providerName || t.fdacs.provider}
                    </Text>
                  </View>
                  {savings > 0 && (
                    <View style={styles.savingsBadge}>
                      <Text style={styles.savingsBadgeText}>
                        {t.fdacs.savePercent.replace(
                          "{percent}",
                          savingsPercent,
                        )}
                      </Text>
                ...
                <TouchableOpacity
                  style={styles.viewDetailBtn}
                  onPress={() =>
                    navigation.navigate("QuoteDetails", { quoteId: cq.id })
                  }
                >
                  <Text style={styles.viewDetailText}>
                    {t.fdacs.viewFullEstimate}
