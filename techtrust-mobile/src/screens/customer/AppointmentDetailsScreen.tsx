...
export default function AppointmentDetailsScreen({ navigation, route }: any) {
  ...
  const handleConfirm = async () => {
    try {
      ...
      Alert.alert(
        t.common.error,
        error.response?.data?.message || t.fdacs.couldNotConfirm,
      );
    } finally {
      setProcessing(false);
    }
  ...
  const handleCheckIn = async () => {
    try {
      ...
      Alert.alert(
        t.common.error,
        error.response?.data?.message || t.fdacs.couldNotCheckIn,
      );
    } finally {
      setProcessing(false);
    }
  ...
  const handleComplete = async () => {
    Alert.alert(t.fdacs.confirmTitle, t.fdacs.confirmMessage, [
      { text: t.common.cancel, style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert(
              t.common.error,
              error.response?.data?.message || t.fdacs.couldNotComplete,
            );
          } finally {
            setProcessing(false);
          }
  ...
  const handleCancel = async () => {
    Alert.alert(t.fdacs.cancelTitle, t.fdacs.cancelMessage, [
      { text: t.common.no, style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            setProcessing(true);
            await fdacsService.cancelAppointment(
              appointmentId,
              t.fdacs.cancelledByUser,
            );
            ...
            Alert.alert(
              t.common.error,
              error.response?.data?.message || t.fdacs.couldNotCancel,
            );
          } finally {
            setProcessing(false);
          }
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      case "REQUESTED":
        return {
          label: t.fdacs.statusRequested,
          color: "#f59e0b",
          bgColor: "#fef3c7",
        };
      case "CONFIRMED":
        return {
          label: t.fdacs.statusConfirmed,
          color: "#3b82f6",
          bgColor: "#dbeafe",
        };
      case "PROVIDER_EN_ROUTE":
        return {
          label: t.fdacs.statusProviderEnRoute,
          color: "#8b5cf6",
          bgColor: "#ede9fe",
        };
      case "IN_PROGRESS":
      ...
      case "COMPLETED":
        return {
          label: t.fdacs.statusCompleted,
          color: "#10b981",
          bgColor: "#d1fae5",
        };
      case "CANCELLED":
        return {
          label: t.fdacs.statusCancelled,
          color: "#ef4444",
          bgColor: "#fee2e2",
        };
      case "NO_SHOW":
        return {
          label: t.fdacs.statusNoShow,
          color: "#6b7280",
          bgColor: "#f3f4f6",
        };
      default:
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        {/* Status */}
        <View
          style={[styles.statusBar, { backgroundColor: statusInfo.bgColor }]}
        >
        ...
        <View style={styles.card}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.rowText}>
              {appointment.locationType === "ON_SITE"
                ? t.fdacs.onSiteCustomer
                : t.fdacs.atProviderShop}
            </Text>
          </View>
          {appointment.address && (
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            {isProvider ? t.fdacs.customer : t.fdacs.provider}
          </Text>
          {isProvider ? (
            <Text style={styles.rowText}>
              {appointment.customer?.fullName || t.fdacs.na}
            </Text>
          ) : (
            <>
              <Text style={styles.rowText}>
                {appointment.provider?.fullName || t.fdacs.na}
              </Text>
        ...
        {(Number(appointment.diagnosticFee) > 0 ||
          ...
          <View style={styles.card}>
            ...
            {appointment.feeWaivedOnService && (
              <Text style={styles.waivedNote}>{t.fdacs.feeWaivedNote}</Text>
            )}
          </View>
        )}
