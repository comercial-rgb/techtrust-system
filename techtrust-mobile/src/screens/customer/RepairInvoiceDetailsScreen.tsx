...
export default function RepairInvoiceDetailsScreen({ route, navigation }: any) {
  ...
  const [invoice, setInvoice] = useState<fdacsService.RepairInvoice | null>(null);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
          ...
          {lineItems.length > 0 ? (
            lineItems.map((item: any, idx: number) => (
              <View key={idx} style={styles.lineItem}>
                ...
                <Text style={styles.lineItemAmount}>${Number(item.amount).toFixed(2)}</Text>
              </View>
            ))
          ) : (
        ...
        {/* FDACS Compliance */}
        <View style={[styles.section, { backgroundColor: '#f0f9ff', borderColor: '#1976d2' }]}>
...
const styles = StyleSheet.create({
  ...
  lineItemAmount: { fontSize: 14, fontWeight: '600', color: '#111827', marginLeft: 12 },
  supplementTag: {
    backgroundColor: '#fef3c7', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6, alignSelf: 'flex-start', marginTop: 4,
  },
  ...
  totalFinal: {
    marginTop: 8, paddingTop: 12, borderTopWidth: 2, borderTopColor: '#e5e7eb', marginBottom: 0,
  },
