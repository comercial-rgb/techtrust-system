...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, TextInput, Alert, Linking, Platform, Modal, FlatList, ActivityIndicator } from 'react-native';
...
export default function CreateRequestScreen({ navigation }: any) {
  ...
  const preSelectedProviderFromLanding: PreSelectedProvider | null = route.params?.preSelectedProvider || null;
  const specialOfferFromLanding: SpecialOffer | null = route.params?.specialOffer || null;
  
  const [selectedVehicle, setSelectedVehicle] = useState<string>('1');
  // Auto-select service type from special offer if present
  const [selectedService, setSelectedService] = useState<string>(specialOfferFromLanding?.serviceType || '');
  const [title, setTitle] = useState(specialOfferFromLanding ? specialOfferFromLanding.title : '');
  const [description, setDescription] = useState(specialOfferFromLanding ? `${specialOfferFromLanding.description} (${specialOfferFromLanding.discount})` : '');
  ...
  const [activeServiceInfo, setActiveServiceInfo] = useState<{orderNumber: string; title: string; status: string} | null>(null);
  
  // Favorite Provider Selection
  const [showProviderModal, setShowProviderModal] = useState(false);
  const [selectedProvider, setSelectedProvider] = useState<FavoriteProvider | null>(
    preSelectedProviderFromLanding
      ...
      : preSelectedProviderId && preSelectedProviderName 
        ? { id: preSelectedProviderId, businessName: preSelectedProviderName, rating: 0, totalServices: 0, specialty: '' }
        : null
  );
  ...
  async function handleGetLocation() {
    ...
    Alert.alert(
      ...
      t.createRequest?.shareLocationMessage || 'Allow TechTrust to access your location to share with the service provider?',
      [
        ...
        { 
          ...
          onPress: () => {
            ...
            Alert.alert(t.common?.success || 'Success', t.createRequest?.locationShared || 'Your location will be shared with the provider');
          }
        },
      ]
    );
  ...
  async function handleSubmit() {
    ...
    const providerName = selectedProvider?.businessName || preSelectedProviderName;
    const successMessage = providerName 
      ...
      : (t.createRequest?.quotesWithin || 'You will receive quotes within 48 hours.');
    
    Alert.alert(t.createRequest?.submitted || 'Request Submitted!', successMessage, [
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.noPaymentContainer}>
          ...
          <Text style={styles.noPaymentDescription}>
            {t.createRequest?.paymentRequiredDesc || 'To create a service request, you need to add a payment method first. This ensures secure payment processing when you accept a quote.'}
          </Text>
          
          <View style={styles.noPaymentFeatures}>
            <View style={styles.featureRow}>
              <Ionicons name="shield-checkmark" size={20} color="#10b981" />
              <Text style={styles.featureText}>{t.createRequest?.securePayment || 'Secure payment processing'}</Text>
            </View>
            <View style={styles.featureRow}>
              <Ionicons name="lock-closed" size={20} color="#10b981" />
              <Text style={styles.featureText}>{t.createRequest?.fundsHeld || 'Funds held until service completion'}</Text>
            </View>
            <View style={styles.featureRow}>
              <Ionicons name="refresh" size={20} color="#10b981" />
              <Text style={styles.featureText}>{t.createRequest?.easyRefund || 'Easy refunds if service cancelled'}</Text>
            </View>
          </View>
          ...
          >
            ...
            <Text style={styles.addPaymentText}>{t.createRequest?.addPaymentMethod || 'Add Payment Method'}</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={styles.laterText}>{t.createRequest?.doLater || 'I\'ll do this later'}</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.noPaymentContainer}>
          ...
          <Text style={styles.noPaymentTitle}>
            {t.createRequest?.activeServiceExists || 'Active Service in Progress'}
          </Text>
          <Text style={styles.noPaymentDescription}>
            {t.createRequest?.activeServiceDesc || 'You have an ongoing service that needs to be completed before creating a new request. Please complete or cancel your current service first.'}
          </Text>
          
          <View style={styles.activeServiceCard}>
            <View style={styles.activeServiceHeader}>
              <View style={[styles.activeServiceStatus, { backgroundColor: '#fef3c7' }]}>
                <Ionicons name="construct" size={16} color="#f59e0b" />
                <Text style={[styles.activeServiceStatusText, { color: '#f59e0b' }]}>
                  {getStatusText(activeServiceInfo.status)}
                </Text>
              </View>
              <Text style={styles.activeServiceOrder}>#{activeServiceInfo.orderNumber}</Text>
            </View>
            <Text style={styles.activeServiceTitle}>{activeServiceInfo.title}</Text>
          </View>
          ...
          >
            ...
            <Text style={styles.addPaymentText}>{t.createRequest?.viewActiveService || 'View Active Service'}</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity 
            ...
            onPress={() => navigation.goBack()}
          >
            <Text style={styles.laterText}>{t.common?.goBack || 'Go Back'}</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={styles.paymentMethodCard}>
          <View style={styles.paymentMethodInfo}>
            ...
            <View style={{ flex: 1, marginLeft: 10 }}>
              <Text style={styles.paymentMethodLabel}>{t.createRequest?.paymentMethod || 'Payment Method'}</Text>
            ...
            >
              <Text style={styles.changePaymentText}>{t.common?.change || 'Change'}</Text>
            </TouchableOpacity>
          </View>
        ...
        {(selectedProvider || preSelectedProviderName) && (
          <View style={styles.selectedProviderBanner}>
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              ...
              <Text style={styles.selectedProviderName}>{selectedProvider?.businessName || preSelectedProviderName}</Text>
            </View>
        ...
        {serviceLocation === 'roadside' && (
          <View style={styles.roadsideContainer}>
            {!shareLocation ? (
              <TouchableOpacity style={styles.shareLocationBtn} onPress={handleGetLocation}>
                <Ionicons name="location" size={24} color="#fff" />
                <Text style={styles.shareLocationText}>{t.createRequest?.shareMyLocation || 'Share My Location'}</Text>
              </TouchableOpacity>
            ) : (
              <View style={styles.locationSharedCard}>
                <View style={styles.locationSharedHeader}>
                  <Ionicons name="checkmark-circle" size={24} color="#10b981" />
                  <Text style={styles.locationSharedTitle}>{t.createRequest?.locationReady || 'Location Ready'}</Text>
                </View>
                <Text style={styles.locationSharedText}>
                  {t.createRequest?.locationWillBeShared || 'Your real-time location will be shared with the provider'}
                </Text>
                ...
                <TouchableOpacity style={styles.viewOnMapBtn} onPress={openInMaps}>
                  <Ionicons name="map" size={18} color="#1976d2" />
                  <Text style={styles.viewOnMapText}>{t.createRequest?.viewOnMap || 'View on Map'}</Text>
                </TouchableOpacity>
              </View>
            )}
            <View style={styles.roadsideInfo}>
              ...
              <Text style={styles.roadsideInfoText}>
                {t.createRequest?.roadsideNote || 'Provider will receive your live location via Google Maps'}
              </Text>
            </View>
          </View>
        )}
        ...
        {/* Favorite Provider Selection */}
        <Text style={styles.sectionTitle}>{t.createRequest?.sendToFavorite || 'Send to Favorite Provider'}</Text>
        <TouchableOpacity 
        ...
        >
          {selectedProvider ? (
            <View style={styles.selectedProviderRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.selectedProviderNameText}>{selectedProvider.businessName}</Text>
                {selectedProvider.rating > 0 && (
                  <View style={styles.ratingRowSmall}>
                    <Ionicons name="star" size={12} color="#fbbf24" />
                    <Text style={styles.ratingTextSmall}>{selectedProvider.rating}</Text>
                  </View>
                )}
              </View>
              <TouchableOpacity 
            ...
            <View style={styles.selectProviderPlaceholder}>
              ...
              <Text style={styles.selectProviderText}>
                {t.createRequest?.selectFavoriteProvider || 'Select a favorite provider (optional)'}
              </Text>
        ...
        <Text style={styles.favoriteHint}>
          {t.createRequest?.favoriteHint || 'Request will be sent directly to this provider instead of all providers'}
        </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{t.createRequest?.selectFavoriteTitle || 'Select Favorite Provider'}</Text>
            ...
            {favoriteProviders.length === 0 ? (
              <View style={styles.emptyFavorites}>
                ...
                <Text style={styles.emptyFavoritesSubtext}>
                  {t.createRequest?.addFavoritesHint || 'Add providers to your favorites from the Profile tab'}
                </Text>
              </View>
            ) : (
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={[
                      ...
                      selectedProvider?.id === item.id && styles.providerListItemSelected
                    ]}
                    onPress={() => {
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={styles.providerListName}>{item.businessName}</Text>
                      <View style={styles.providerListMeta}>
                        <Ionicons name="star" size={14} color="#fbbf24" />
                        <Text style={styles.providerListRating}>{item.rating}</Text>
                        <Text style={styles.providerListServices}>• {item.totalServices} {t.common?.services || 'services'}</Text>
                      </View>
                      <Text style={styles.providerListSpecialty}>{item.specialty}</Text>
                    </View>
            ...
            >
              ...
              <Text style={styles.sendToAllText}>{t.createRequest?.sendToAll || 'Send to all providers'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  content: { padding: 16 },
  // Selected Provider Banner
  selectedProviderBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#eff6ff', padding: 16, borderRadius: 12, marginBottom: 20, borderWidth: 1, borderColor: '#1976d2' },
  ...
  locationCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 14, borderRadius: 12, marginBottom: 6, borderWidth: 2, borderColor: '#e5e7eb' },
  ...
  shareLocationBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#ef4444', paddingVertical: 16, borderRadius: 12 },
  shareLocationText: { fontSize: 16, fontWeight: '600', color: '#fff' },
  locationSharedCard: { backgroundColor: '#d1fae5', padding: 16, borderRadius: 12 },
  locationSharedHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
  locationSharedTitle: { fontSize: 16, fontWeight: '600', color: '#10b981' },
  locationSharedText: { fontSize: 14, color: '#065f46', marginBottom: 8 },
  locationCoords: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 12 },
  locationCoordsText: { fontSize: 13, color: '#6b7280' },
  viewOnMapBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#fff', paddingVertical: 10, borderRadius: 8 },
  viewOnMapText: { fontSize: 14, fontWeight: '600', color: '#1976d2' },
  roadsideInfo: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, backgroundColor: '#fef3c7', padding: 12, borderRadius: 8, marginTop: 12 },
  ...
  favoriteProviderSelector: { backgroundColor: '#fff', borderRadius: 12, borderWidth: 2, borderColor: '#e5e7eb', marginBottom: 8 },
  selectedProviderRow: { flexDirection: 'row', alignItems: 'center', padding: 16 },
  providerAvatarSmall: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#dbeafe', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  selectedProviderNameText: { fontSize: 16, fontWeight: '600', color: '#111827' },
  ratingRowSmall: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 2 },
  ratingTextSmall: { fontSize: 12, color: '#6b7280' },
  clearProviderBtn: { padding: 4 },
  selectProviderPlaceholder: { flexDirection: 'row', alignItems: 'center', padding: 16, gap: 12 },
  selectProviderText: { flex: 1, fontSize: 14, color: '#9ca3af' },
  favoriteHint: { fontSize: 12, color: '#6b7280', marginBottom: 24, fontStyle: 'italic' },
  // Provider Modal
  ...
  providerListItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f8fafc', padding: 16, borderRadius: 12, marginBottom: 8, borderWidth: 2, borderColor: 'transparent' },
  providerListItemSelected: { borderColor: '#1976d2', backgroundColor: '#eff6ff' },
  providerListAvatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#dbeafe', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  providerListName: { fontSize: 16, fontWeight: '600', color: '#111827' },
  providerListMeta: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 2 },
  ...
  emptyFavoritesText: { fontSize: 16, fontWeight: '600', color: '#6b7280', marginTop: 12 },
  emptyFavoritesSubtext: { fontSize: 14, color: '#9ca3af', textAlign: 'center', marginTop: 8 },
  sendToAllBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginHorizontal: 16, paddingVertical: 14, borderRadius: 12, backgroundColor: '#eff6ff' },
  ...
  noPaymentContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 32 },
  noPaymentIcon: { width: 120, height: 120, borderRadius: 60, backgroundColor: '#f3f4f6', justifyContent: 'center', alignItems: 'center', marginBottom: 24 },
  noPaymentTitle: { fontSize: 22, fontWeight: '700', color: '#111827', marginBottom: 12, textAlign: 'center' },
  noPaymentDescription: { fontSize: 15, color: '#6b7280', textAlign: 'center', lineHeight: 22, marginBottom: 24 },
  noPaymentFeatures: { width: '100%', backgroundColor: '#f8fafc', borderRadius: 12, padding: 16, marginBottom: 24 },
  featureRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 },
  featureText: { fontSize: 14, color: '#374151' },
  addPaymentBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#1976d2', paddingVertical: 16, paddingHorizontal: 32, borderRadius: 12, width: '100%' },
  ...
  activeServiceCard: { width: '100%', backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 24, borderWidth: 1, borderColor: '#fcd34d' },
  activeServiceHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 },
  activeServiceStatus: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  ...
  // Payment Method Card
  paymentMethodCard: { backgroundColor: '#f0fdf4', borderRadius: 12, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: '#bbf7d0' },
  ...
  paymentMethodNote: { fontSize: 12, color: '#166534', marginTop: 10, lineHeight: 18 },
