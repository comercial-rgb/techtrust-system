...
export default function WorkOrderDetailsScreen({ navigation, route }: any) {
  ...
  async function loadDetails() {
    ...
    // Mock data based on workOrderId to match CustomerWorkOrdersScreen data
    const mockWorkOrders: Record<string, any> = {
      '1': {
        ...
        description: 'Complete 30,000 mile preventive inspection including oil change, filters, brake and suspension check.',
        createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
        scheduledDate: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000).toISOString(),
        completedAt: null,
        provider: { id: 'provider-1', businessName: 'Auto Center Express', phone: '+1 (407) 555-1234', rating: 4.8, address: '123 Auto Lane, Orlando, FL' },
        vehicle: { make: 'Toyota', model: 'Corolla', year: 2019, plateNumber: 'XYZ5678' },
        timeline: [
          { status: 'QUOTE_ACCEPTED', label: t.workOrder?.quoteAccepted || 'Quote Accepted', date: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'SCHEDULED', label: t.workOrder?.serviceScheduled || 'Service Scheduled', date: new Date(Date.now() - 1.5 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'IN_PROGRESS', label: t.workOrder?.serviceStarted || 'Service Started', date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() },
        ],
      },
      '2': {
        id: '2',
        ...
        provider: { id: 'provider-2', businessName: "John's Auto Repair", phone: '+1 (407) 555-5678', rating: 4.5, address: '456 Mechanic St, Orlando, FL' },
        vehicle: { make: 'Honda', model: 'Civic', year: 2020, plateNumber: 'ABC1234' },
        timeline: [
          { status: 'QUOTE_ACCEPTED', label: t.workOrder?.quoteAccepted || 'Quote Accepted', date: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'SCHEDULED', label: t.workOrder?.serviceScheduled || 'Service Scheduled', date: new Date(Date.now() - 3.5 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'IN_PROGRESS', label: t.workOrder?.serviceStarted || 'Service Started', date: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'COMPLETED', label: t.workOrder?.serviceCompleted || 'Service Completed', date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'AWAITING_PAYMENT', label: t.workOrder?.awaitingPayment || 'Awaiting Payment', date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() },
        ],
      },
      '3': {
        id: '3',
        ...
        vehicle: { make: 'Honda', model: 'Civic', year: 2020, plateNumber: 'ABC1234' },
        timeline: [
          { status: 'QUOTE_ACCEPTED', label: t.workOrder?.quoteAccepted || 'Quote Accepted', date: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'SCHEDULED', label: t.workOrder?.serviceScheduled || 'Service Scheduled', date: new Date(Date.now() - 7.5 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'IN_PROGRESS', label: t.workOrder?.serviceStarted || 'Service Started', date: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'COMPLETED', label: t.workOrder?.serviceCompleted || 'Service Completed', date: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'PAID', label: t.workOrder?.paymentReceived || 'Payment Received', date: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString() },
        ],
      },
      '4': {
        id: '4',
        ...
        provider: { id: 'provider-3', businessName: 'Premium Auto Shop', phone: '+1 (407) 555-9999', rating: 4.9, address: '789 Service Blvd, Orlando, FL' },
        vehicle: { make: 'Toyota', model: 'Corolla', year: 2019, plateNumber: 'XYZ5678' },
        timeline: [
          { status: 'QUOTE_ACCEPTED', label: t.workOrder?.quoteAccepted || 'Quote Accepted', date: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'SCHEDULED', label: t.workOrder?.serviceScheduled || 'Service Scheduled', date: new Date(Date.now() - 14.5 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'IN_PROGRESS', label: t.workOrder?.serviceStarted || 'Service Started', date: new Date(Date.now() - 13 * 24 * 60 * 60 * 1000).toISOString() },
          { status: 'COMPLETED', label: t.workOrder?.serviceCompleted || 'Service Completed', date: new Date(Date.now() - 12 * 24 * 60 * 60 * 1000).toISOString() },
    ...
    const woData = mockWorkOrders[workOrderId] || mockWorkOrders['1'];
    
    // Add common lineItems and warranty
    // Add pending additional parts for IN_PROGRESS orders (mock)
    const pendingParts = woData.status === 'IN_PROGRESS' ? {
      pendingAdditionalParts: {
        id: 'apr-001',
        status: 'pending',
        reason: 'During inspection, we found worn spark plugs that need replacement for optimal engine performance.',
        requestedAt: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(),
        parts: [
          { description: 'Spark Plug - NGK Iridium', quantity: 4, unitPrice: 18.50 },
          { description: 'Ignition Coil - Bosch', quantity: 1, unitPrice: 65.00 },
        ],
        totalAmount: 139.00,
      }
    } : {};
    ...
    setWorkOrder({
      ...
      warranty: {
        ...
        terms: t.workOrder?.warrantyTerms || 'Warranty valid upon presentation of service receipt.',
      },
    });
    setLoading(false);
  }
