...
export default function PaymentScreen({ navigation, route }: any) {
  ...
  const [savedMethods, setSavedMethods] = useState<paymentService.PaymentMethod[]>([]);
  const [selectedMethodId, setSelectedMethodId] = useState<string | null>(null);
  const [paymentIntent, setPaymentIntent] = useState<paymentService.CreatePaymentIntentResponse | null>(null);
  ...
  async function handlePayment() {
    ...
    if (!selectedMethodId && savedMethods.length > 0) {
      Alert.alert(t.common?.error || 'Error', t.payment?.selectMethod || 'Please select a payment method');
      return;
    }
    ...
    if (savedMethods.length === 0) {
      Alert.alert(
        ...
        t.payment?.addPaymentMethodFirst || 'Please add a payment method first.',
        [
          { text: t.common?.cancel || 'Cancel' },
          { text: t.payment?.addCard || 'Add Card', onPress: () => navigation.navigate('PaymentMethods', { addCardMode: true }) },
        ]
      );
      return;
    }
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>{t.payment?.loadingPayment || 'Preparing payment...'}</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.summaryCard}>
          ...
          {orderNumber && <Text style={styles.summaryOrder}>#{orderNumber}</Text>}
        ...
        {breakdown && (
          <View style={styles.breakdownCard}>
            <Text style={styles.breakdownTitle}>{t.payment?.breakdown || 'Payment Breakdown'}</Text>
            <View style={styles.breakdownRow}>
              <Text style={styles.breakdownLabel}>{t.payment?.serviceAmount || 'Service'}</Text>
              <Text style={styles.breakdownValue}>${breakdown.serviceAmount.toFixed(2)}</Text>
            </View>
            <View style={styles.breakdownRow}>
              <Text style={styles.breakdownLabel}>{t.payment?.platformFee || 'Platform fee'}</Text>
              <Text style={styles.breakdownValue}>${breakdown.platformFee.toFixed(2)}</Text>
            </View>
            <View style={styles.breakdownRow}>
              <Text style={styles.breakdownLabel}>{t.payment?.processingFee || 'Processing fee'}</Text>
              <Text style={styles.breakdownValue}>${breakdown.stripeFee.toFixed(2)}</Text>
            </View>
            <View style={[styles.breakdownRow, styles.breakdownTotal]}>
              <Text style={styles.breakdownTotalLabel}>{t.payment?.total || 'Total'}</Text>
              <Text style={styles.breakdownTotalValue}>${breakdown.totalAmount.toFixed(2)}</Text>
            </View>
            <View style={styles.providerReceives}>
              <Ionicons name="information-circle-outline" size={14} color="#6b7280" />
              <Text style={styles.providerReceivesText}>
                {t.payment?.providerReceives || 'Provider receives'}: ${breakdown.providerWillReceive.toFixed(2)}
              </Text>
            </View>
          </View>
        )}
        ...
        {savedMethods.length === 0 ? (
          <View style={styles.noMethodsContainer}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => navigation.navigate('PaymentMethods', { addCardMode: true })}
            >
              <Ionicons name="add-circle" size={20} color="#1976d2" />
              <Text style={styles.addMethodBtnText}>{t.payment?.addPaymentMethod || 'Add Payment Method'}</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <View style={styles.methodsContainer}>
            {savedMethods.map(method => (
              ...
              >
                <Ionicons 
                  name={method.type === 'pix' ? 'qr-code' : getCardIcon(method.cardBrand) as any} 
                  size={24} 
                  color={selectedMethodId === method.id ? '#1976d2' : '#6b7280'} 
                />
                <View style={styles.methodInfo}>
                  {method.type === 'pix' ? (
                    <Text style={[styles.methodLabel, selectedMethodId === method.id && styles.methodLabelSelected]}>
                      PIX - {method.pixKey}
                    </Text>
                  ) : (
                    <>
                      <Text style={[styles.methodLabel, selectedMethodId === method.id && styles.methodLabelSelected]}>
                        {(method.cardBrand || method.type).toUpperCase()} •••• {method.cardLast4}
                      </Text>
                      {method.cardExpMonth && method.cardExpYear && (
                        <Text style={styles.methodExpiry}>
                          Exp: {String(method.cardExpMonth).padStart(2, '0')}/{String(method.cardExpYear).slice(-2)}
                        </Text>
                      )}
                    </>
                  )}
                  {method.isDefault && (
                    <Text style={styles.defaultBadge}>{t.payment?.defaultMethod || 'Default'}</Text>
                  )}
                </View>
                {selectedMethodId === method.id && <Ionicons name="checkmark-circle" size={20} color="#1976d2" />}
              </TouchableOpacity>
            ))}
            <TouchableOpacity 
            ...
            >
              ...
              <Text style={styles.addAnotherText}>{t.payment?.addAnother || 'Add another method'}</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        <View style={styles.securityInfo}>
          ...
          <Text style={styles.securityText}>{t.payment?.securePayment || '100% secure payment powered by Stripe'}</Text>
        </View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity 
          style={[styles.payBtn, (processing || savedMethods.length === 0) && styles.payBtnDisabled]} 
        ...
        >
          {processing ? (
            ...
            <>
              ...
              <Text style={styles.payText}>{t.payment?.pay || 'Pay'} ${displayAmount.toFixed(2)}</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  retryBtn: { backgroundColor: '#1976d2', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 10, marginTop: 8 },
  ...
  // Breakdown
  breakdownCard: { backgroundColor: '#fff', marginHorizontal: 16, marginBottom: 16, padding: 16, borderRadius: 12, borderWidth: 1, borderColor: '#e5e7eb' },
  breakdownTitle: { fontSize: 14, fontWeight: '600', color: '#374151', marginBottom: 12 },
  ...
  providerReceives: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 8 },
  providerReceivesText: { fontSize: 12, color: '#6b7280' },
  // Section
  sectionTitle: { fontSize: 16, fontWeight: '600', color: '#111827', marginHorizontal: 16, marginBottom: 12, marginTop: 8 },
  // No methods
  noMethodsContainer: { alignItems: 'center', padding: 32, backgroundColor: '#fff', marginHorizontal: 16, borderRadius: 12, borderWidth: 1, borderColor: '#e5e7eb' },
  noMethodsText: { fontSize: 14, color: '#6b7280', marginTop: 8, marginBottom: 16 },
  addMethodBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#eff6ff', paddingHorizontal: 20, paddingVertical: 12, borderRadius: 10 },
  ...
  defaultBadge: { fontSize: 11, color: '#10b981', fontWeight: '600', marginTop: 2 },
  addAnotherMethod: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 12 },
