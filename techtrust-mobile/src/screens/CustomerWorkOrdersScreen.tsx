...
export default function CustomerWorkOrdersScreen({ navigation }: any) {
  ...
  const stats = {
    ...
    quotesReceived: serviceRequests.filter(r => r.status === 'QUOTES_RECEIVED').length,
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('Dashboard', { screen: 'CreateRequest' })}
        >
      ...
      <FadeInView delay={0}>
        <View style={styles.tabsContainer}>
          <TouchableOpacity 
            ...
            onPress={() => { setActiveTab('requests'); setFilter('all'); }}
          >
            <Ionicons 
              name="pricetags" 
              size={18} 
          ...
          <TouchableOpacity 
            ...
            onPress={() => { setActiveTab('orders'); setFilter('all'); }}
          >
            <Ionicons 
              name="construct" 
              size={18} 
            ...
            <>
              <View style={[styles.statCard, { backgroundColor: '#dbeafe' }]}>
                ...
                <Text style={[styles.statValue, { color: '#1e40af' }]}>
                  {serviceRequests.filter(r => r.status === 'SEARCHING').length}
                </Text>
                <Text style={styles.statLabel}>{t.serviceRequest?.searching || 'Searching'}</Text>
              </View>
              <View style={[styles.statCard, { backgroundColor: '#fef3c7' }]}>
                <Ionicons name="pricetag" size={20} color="#f59e0b" />
                <Text style={[styles.statValue, { color: '#b45309' }]}>{stats.quotesReceived}</Text>
                <Text style={styles.statLabel}>{t.serviceRequest?.withQuotes || 'With Quotes'}</Text>
              </View>
              <View style={[styles.statCard, { backgroundColor: '#d1fae5' }]}>
                <Ionicons name="document-text" size={20} color="#10b981" />
                <Text style={[styles.statValue, { color: '#047857' }]}>{stats.openRequests}</Text>
                <Text style={styles.statLabel}>{t.serviceRequest?.totalOpen || 'Total Open'}</Text>
              </View>
            </>
          ) : (
            <>
      ...
      <FadeInView delay={100}>
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder={activeTab === 'requests' ? (t.serviceRequest?.searchRequests || 'Search requests...') : (t.workOrder?.searchServices || 'Search services...')}
        ...
        {activeTab === 'requests' ? (
          ...
          filteredRequests.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>{t.serviceRequest?.noOpenRequests || 'No open requests'}</Text>
              <Text style={styles.emptySubtitle}>
                {t.serviceRequest?.createNewRequest || 'Create a new request to receive quotes'}
              </Text>
              <TouchableOpacity 
              ...
              >
                <Ionicons name="add" size={20} color="#fff" />
                <Text style={styles.emptyButtonText}>{t.common?.add || 'New Request'}</Text>
              </TouchableOpacity>
            </View>
          ) : (
            filteredRequests.map((request, index) => {
              ...
              return (
                <FadeInView key={request.id} delay={200 + index * 50}>
                  <ScalePress onPress={() => navigation.navigate('Dashboard', { 
                    ...
                    <View style={styles.orderCard}>
                      ...
                      <View style={styles.orderFooter}>
                        ...
                        <Text style={styles.dateText}>{formatTimeAgo(request.createdAt)} {t.common?.ago || 'ago'}</Text>
                      </View>
                      ...
                      {request.quotesCount > 0 && (
                        <View style={styles.quotesAlert}>
                          ...
                          <Text style={styles.quotesAlertText}>
                            {request.quotesCount} {t.common?.quotesCount || 'quote(s) received'}
                          </Text>
                  ...
                  <ScalePress onPress={() => navigation.navigate('WorkOrderDetails', { workOrderId: wo.id })}>
                    <View style={styles.orderCard}>
                      ...
                      <View style={styles.orderFooter}>
                        ...
                        <Text style={styles.amount}>R${wo.finalAmount.toFixed(2)}</Text>
                      </View>
