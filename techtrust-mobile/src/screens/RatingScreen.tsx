    ...
    try {
      ...
      if (err?.response?.data?.code === 'INSUFFICIENT_BALANCE') {
        Alert.alert(
          ...
          t.rating?.insufficientBalance || 'Insufficient wallet balance for tip. Review will still be submitted.',
        );
      }
      return false;
    } finally {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      Alert.alert(
        ...
        (t.rating?.reviewSubmitted || 'Your review has been submitted successfully.') + tipMsg,
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={styles.tipSection}>
          <View style={styles.tipHeader}>
            ...
            <Text style={styles.tipTitle}>{t.rating?.addTip || 'Add a Tip'}</Text>
            <Text style={styles.tipOptional}>({t.common?.optional || 'Optional'})</Text>
          </View>
          <Text style={styles.tipDescription}>
            {t.rating?.tipDescription || 'Show your appreciation with a tip. 100% goes to the provider.'}
          </Text>
          ...
          <View style={styles.tipPresetsContainer}>
            {TIP_PRESETS.map((preset) => (
              <TouchableOpacity
                ...
                style={[
                  styles.tipPresetBtn,
                  tipAmount === preset && !customTipAmount && styles.tipPresetBtnSelected,
                ]}
                onPress={() => handleSelectTipPreset(preset)}
              >
                <Text style={[
                  styles.tipPresetText,
                  tipAmount === preset && !customTipAmount && styles.tipPresetTextSelected,
                ]}>
                  ${preset}
                </Text>
              </TouchableOpacity>
          ...
          <View style={styles.tipCustomRow}>
            <Text style={styles.tipCustomLabel}>{t.rating?.customAmount || 'Custom'}: $</Text>
            <TextInput
          ...
          {tipAmount && tipAmount > 0 ? (
            <TextInput
              ...
              placeholder={t.rating?.tipMessagePlaceholder || 'Add a message (optional)'}
              placeholderTextColor="#9ca3af"
              value={tipMessage}
          ...
          {!tipAmount && (
            <Text style={styles.tipNoTipNote}>
              {t.rating?.noTipNote || "No worries! Tipping is completely optional."}
            </Text>
          )}
        </View>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          style={[styles.submitBtn, (rating === 0 || submitting || sendingTip) && styles.submitBtnDisabled]}
...
const styles = StyleSheet.create({
  ...
  // Tip styles
  tipSection: { backgroundColor: '#fff', borderRadius: 16, padding: 20, marginBottom: 24, borderWidth: 1, borderColor: '#e5e7eb' },
  tipHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
  tipTitle: { fontSize: 17, fontWeight: '700', color: '#111827' },
  tipOptional: { fontSize: 13, color: '#9ca3af', fontWeight: '400' },
  tipDescription: { fontSize: 13, color: '#6b7280', marginBottom: 16, lineHeight: 18 },
  tipPresetsContainer: { flexDirection: 'row', justifyContent: 'space-between', gap: 10, marginBottom: 12 },
  tipPresetBtn: { flex: 1, backgroundColor: '#f3f4f6', paddingVertical: 14, borderRadius: 12, alignItems: 'center', borderWidth: 1.5, borderColor: '#e5e7eb' },
  ...
  tipCustomRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  tipCustomLabel: { fontSize: 15, fontWeight: '500', color: '#374151', marginRight: 4 },
  tipCustomInput: { flex: 1, backgroundColor: '#f9fafb', borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 16, color: '#111827' },
  tipMessageInput: { backgroundColor: '#f9fafb', borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, color: '#111827', marginBottom: 4 },
  tipNoTipNote: { fontSize: 12, color: '#9ca3af', textAlign: 'center', fontStyle: 'italic', marginTop: 4 },
  tipSummary: { fontSize: 13, fontWeight: '500', color: '#10b981', marginBottom: 8 },
});
