...
export const updateTechnician = async (id: string, data: {
  fullName?: string;
  role?: string;
  epa609CertNumber?: string;
  epa609CertType?: string;
...
export const batchUpsertInsurance = async (policies: Array<{
  type: string;
  hasCoverage: boolean;
  carrierName?: string;
  policyNumber?: string;
...
export const checkRiskAcceptance = async (providerId: string, serviceType: string): Promise<RiskAcceptanceCheck> => {
...
export const getProviderComplianceBadge = (status: string) => {
  switch (status) {
    case 'VERIFIED':
      return { label: 'Verified', color: '#16a34a', icon: 'shield-checkmark' as const, bgColor: '#f0fdf4' };
    case 'PENDING':
      return { label: 'Pending', color: '#d97706', icon: 'time' as const, bgColor: '#fffbeb' };
    case 'RESTRICTED':
      return { label: 'Restricted', color: '#dc2626', icon: 'warning' as const, bgColor: '#fef2f2' };
    case 'NOT_ELIGIBLE':
      return { label: 'Not Eligible', color: '#6b7280', icon: 'close-circle' as const, bgColor: '#f9fafb' };
    default:
      return { label: 'Unknown', color: '#6b7280', icon: 'help-circle' as const, bgColor: '#f9fafb' };
  }
};

export const getInsuranceStatusLabel = (status: string) => {
  switch (status) {
    case 'INS_VERIFIED': return { label: 'Verified', color: '#16a34a' };
    case 'INS_PROVIDED_UNVERIFIED': return { label: 'Under Review', color: '#d97706' };
    case 'INS_NOT_PROVIDED': return { label: 'Not Provided', color: '#6b7280' };
    case 'INS_EXPIRED': return { label: 'Expired', color: '#dc2626' };
...
export const getComplianceStatusLabel = (status: string) => {
  switch (status) {
    case 'VERIFIED': return { label: 'Verified', color: '#16a34a' };
    case 'PROVIDED_UNVERIFIED': return { label: 'Under Review', color: '#d97706' };
    case 'COMPLIANCE_PENDING': return { label: 'Pending', color: '#d97706' };
